...
func init() {
	mars.OnAppStart(func() {
		// Set the default expiration time.
		defaultExpiration := time.Hour // The default for the default is one hour.
		if expireStr, found := mars.Config.String("cache.expires"); found {
			var err error
		...
		if mars.Config.BoolDefault("cache.memcached", false) && mars.Config.BoolDefault("cache.redis", false) {
			panic("You've configured both memcached and redis, please only include configuration for one cache!")
		}

		// Use memcached?
		if mars.Config.BoolDefault("cache.memcached", false) {
			hosts := strings.Split(mars.Config.StringDefault("cache.hosts", ""), ",")
			if len(hosts) == 0 {
		...
		if mars.Config.BoolDefault("cache.redis", false) {
			hosts := strings.Split(mars.Config.StringDefault("cache.hosts", ""), ",")
			if len(hosts) == 0 {
			...
			password := mars.Config.StringDefault("cache.redis.password", "")
